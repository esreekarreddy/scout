...
import {
  ...
  assertRateLimit,
  assertTrustedOrigin,
  parseScorePatchesRequest,
  readJsonRequest,
} from "@/lib/api-security";
import type { PatchCandidate, PatchExecutionSummary } from "@/lib/types";
...
export async function POST(req: Request) {
  try {
    ...
    assertRateLimit(req, "score-patches", 30);
    const { repo, finding, fixers } = parseScorePatchesRequest(await readJsonRequest(req));
